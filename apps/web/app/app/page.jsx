...
import { fetchMemberDashboardView } from "@/lib/member-workspace";
...
export default async function MemberDashboardPage() {
  ...
  const { error, view } = await fetchMemberDashboardView({ supabase, userId: user.id });
    ...
    <div className="member-rail-stack">
      <article className="member-rail-profile-card">
        <div className="member-rail-profile-top">
          <div className="member-rail-avatar">{view.sidebarUser.initials}</div>
          <div>
            <strong>{view.member.displayName}</strong>
            <p>
              {view.profileSnapshot.role}
              {view.profileSnapshot.country ? ` · ${view.profileSnapshot.country}` : ""}
            </p>
          </div>
        </div>
        {view.profileSnapshot.tags.length ? (
          <div className="member-rail-tag-row">
            {view.profileSnapshot.tags.map((tag) => (
        ...
        <div className="member-rail-profile-stats">
          <div>
            <strong>{view.stats[1].value}</strong>
            <span>Spaces</span>
          </div>
          <div>
            <strong>{view.recentInsights.length}</strong>
            <span>Insights</span>
          </div>
          <div>
            <strong>{view.member.secondaryCohorts.length + (view.member.primaryCohort ? 1 : 0)}</strong>
            <span>Cohorts</span>
          </div>
        </div>
        <div className="member-rail-progress">
          <div className="member-rail-progress-copy">
            ...
            <span>{view.profileSnapshot.completionPercent}% complete</span>
          </div>
          <div className="progress-bar-track" aria-hidden="true">
            <span className="progress-bar-fill" style={{ width: `${view.profileSnapshot.completionPercent}%` }} />
          </div>
        </div>
      </article>
      ...
      <article className="member-rail-card">
        ...
        <div className="member-rail-link-list">
          {view.recentInsights.slice(0, 3).map((item) => (
  ...
  return (
    <MemberWorkspaceShell
      ...
      rightRail={rightRail}
      sidebarUser={view.sidebarUser}
      subtitle={`You have ${view.counts.unreadDiscussions} discussion updates across your current spaces and ${view.stats[2].value} applications in the review queue.`}
      title={`Welcome back, ${view.member.first_name || view.member.displayName}`}
    >
      <div className="member-dashboard-stack">
        ...
        <article className="dashboard-card member-module-card">
          ...
          <div className="member-feed-list">
            {view.recentDiscussions.map((item) => (
        ...
        <article className="dashboard-card member-module-card">
          ...
          <div className="member-review-table">
            ...
            {view.applicationQueue.map((application) => (
